...
The Reacher environment, a double-jointed arm can move to target locations. A reward of +0.1 is provided for each step that the agent's hand is in the goal location. Thus, the agent's goal is to maintain its position at the target location for as many time steps as possible.
...
### Getting Started/starting the environment
...
To train the agent run [Continuous_Control-ddpg.ipynb](Continuous_Control-ddpg.ipynb)
